...
public class Map{
    ...
    public void ChangeAltitude(Aircraft aircraft, int altitude){
        ...
        var currentField = GetFieldsWithAircraft().Find(field => field.Aircraft == aircraft);
        if (currentField == null) return;
        var newField = GameMap[currentField.X, currentField.Y, altitude];
        if (newField.Aircraft != null) return;
        RemoveAircraftFromField(currentField);
        SetAircraftOnField(aircraft, newField);
    }
}